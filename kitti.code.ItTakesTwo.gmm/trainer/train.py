...
class Trainer:
    def __init__(self, hyp_param, vis_tool, metrics, criterion):
        ...
        self.cutmix = CutMix(self.param)
        self.embedd_loss = ContrastLoss(param=self.param)
        self.prototype_enumerator = PrototypeGenerator(hyp_param)

    @staticmethod
    def indices_voxel_coord(x, voxel_coord):
        return x[voxel_coord[:, 0], voxel_coord[:, 1], voxel_coord[:, 2], voxel_coord[:, 3]]

    @staticmethod
    def indices_range_coord(x, proj_coord):
        return x[proj_coord[:, 0], proj_coord[:, 2], proj_coord[:, 1]]
    ...
    def train_epoch(self, epoch, data_loader, model, optimiser):
        ...
        for batch_idx in progress_bar:
            ...
            unlabelled_batch['project_coord'] = unlabelled_batch['project_coord'].cuda(non_blocking=True)
            unlabelled_batch['cover_map'] = unlabelled_batch['cover_map'].cuda(non_blocking=True)
            # produce pseudo logits and embeddings
            (original_voxel_logit, pseudo_range_logit_1), (pseudo_voxel_embed_1, pseudo_range_embed_1) = \
                model.module.produce_pseudo_labels(unlabelled_batch, branch_name='voxel', embedding=True)

            (original_range_logit, pseudo_voxel_logit_2), (pseudo_range_embed_2, pseudo_voxel_embed_2) = \
            ...
            mix_range_input_1, mix_range_coord_1, pseudo_range_label_1, pseudo_range_embed_2 = \
                self.cutmix(unlabelled_batch['range_feature'], unlabelled_batch['project_coord'],
            ...
            mix_voxel_input_2, mix_voxel_coord_2, pseudo_voxel_label_2 = \
                self.cutmix(unlabelled_batch['point_feature'], unlabelled_batch['point_coord'], pseudo_voxel_label_2,
                            modality='voxel')
            ...
            embedd_loss_1 = self.embedd_loss(predict_voxel, anchor_voxel, label_voxel, prototype_voxel,
                                             confidence_voxel)
            ...
            embedd_loss_2 = self.embedd_loss(predict_range, anchor_range, label_range, prototype_range, confidence_range)
            ...
            del anchor_range, contras_range, predict_range, confidence_range,  label_range
            del anchor_voxel, contras_voxel, predict_voxel, confidence_voxel, label_voxel
            del prototype_voxel, prototype_range
    ...
    def validate_epoch(self, epoch, data_loader, model, branch_name):
        ...
        for batch_idx in progress_bar:
            ...
            progress_bar.set_description("epoch {} | modality {} | iou {} acc {}".format(epoch, branch_name, curr_iou, curr_acc))
    ...
    def save_ckpts(self, epoch, model, optimiser, name):
        state = {
            ...
            'optimiser': {'voxel': optimiser['voxel'].state_dict(),
                          'range': optimiser['range'].state_dict()},
            'config': self.param
        }

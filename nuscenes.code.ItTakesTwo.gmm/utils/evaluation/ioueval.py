...
class Metrics:
    def __init__(self, n_classes: int, ignore_index: int = 0, device: str = "cpu"):
        self.voxel_b4 = IouEval(n_classes, ignore_index, device_=device)
        self.range_b4 = IouEval(n_classes, ignore_index, device_=device)
        self.voxel_af = IouEval(n_classes, ignore_index, device_=device)
        self.range_af = IouEval(n_classes, ignore_index, device_=device)
...
class IouEval(torch.nn.Module):
    def __init__(self, n_classes: int, ignore_index: int = 0, device_: str = "cpu"):
        ...
        self.conf_matrix = torch.zeros((self.n_classes, self.n_classes), dtype=torch.long)
        self.ones = None

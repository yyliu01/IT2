...
class FIDNet(nn.Module):
    ...
    def __init__(
        ...
        self.semantic_head = SemanticHead(num_cls=num_cls, input_channel=dim_in, embedding_dim=embedding_dim)
        self.embedding_head = ProjectionHead(dim_in, embedding_dim)

    def forward(self, x, embedding=False):
        # x = batch['range_feature'].cuda(non_blocking=True)
        middle_feature_maps = self.backend(x['range_feature'])  # [bs, 1024, H, W]
        logits = self.semantic_head(middle_feature_maps)  # [bs, cls, H, W]
        if embedding is True:
            z = self.embedding_head(middle_feature_maps)
            z = z[x['project_coord'][:, 0], :, x['project_coord'][:, 2], x['project_coord'][:, 1]]
...
class SemanticHead(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        semantic_output = self.semantic_output(res)  # [bs, cls, H, W]

...
class GaussianMixture(torch.nn.Module):
    ...
    def _init_params(self):
        ...
        if self.covariance_type == "diag":
            if self.var_init is not None:
                ...
                self.var = torch.nn.Parameter(torch.ones(1, self.n_components, self.n_features),
    ...
    def fit(self, x, c, delta=1e-3, n_iter=100, warm_start=False):
        """
        ...
        while (i <= n_iter) and (j >= delta):
            ...
            var_old = self.var
            self.__em(x, c)
            self.log_likelihood = self.__score(x, c)
            ...
            if j <= delta:
                ...
                # print('revert back', flush=True)
                self.__update_mu(mu_old)
                self.__update_var(var_old)
            # else:
            # print('pass now', flush=True)
    ...
    def sample(self, n):
        ...
        for k in torch.arange(self.n_components, device=counts.device)[counts > 0]:
            if self.covariance_type == "diag":
                x_k = self.mu[0, k] + torch.randn(int(counts[k]), self.n_features, device=x.device) * \
    ...
    def _e_step(self, x):
        ...
        """
        x = self.check_size(x)
        estimate_log_prob = self._estimate_log_prob(x)
        weighted_log_prob = estimate_log_prob + torch.log(self.pi)

        log_prob_norm = torch.logsumexp(weighted_log_prob, dim=1, keepdim=True)
        log_resp = weighted_log_prob - log_prob_norm
        return torch.mean(log_prob_norm), log_resp

    def _m_step(self, x, log_resp, c__):
        """
        ...
        denominator = torch.sum(resp, dim=0, keepdim=True) + self.eps
        resp *= c__.unsqueeze(1).unsqueeze(1)
        mu = torch.sum(resp * x, dim=0, keepdim=True) / denominator
        ...
        if self.covariance_type == "full":
            ...
            x2 = (resp * x * x).sum(0, keepdim=True) / denominator
            mu2 = mu * mu
            xmu = (resp * mu * x).sum(0, keepdim=True) / denominator
    ...
    def __em(self, x, c_):
        ...
        pi, mu, var = self._m_step(x, log_resp, c_)
        self.__update_pi(pi)
        self.__update_mu(mu)
        self.__update_var(var)

    def __score(self, x, c_, as_average=True):
        """
    ...
    def __update_mu(self, mu):
        ...
        if mu.size() == (self.n_components, self.n_features):
            ...
            # self.mu.data = mu
            self.mu.data = self.mu.data * self.momentum + mu * (1 - self.momentum)
            self.mu.data = torch.nn.functional.normalize(self.mu.data, p=2, dim=-1)
    ...
    def __update_pi(self, pi):
        ...
        self.pi.data = pi
        # self.pi.data = self.pi.data * self.momentum + pi * (1 - self.momentum)

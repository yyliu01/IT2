...
from data.lidar import LidarPointCloud
...
class NuscenesLidarMethods:

    @staticmethod
    def splits(split: str, db: NuScenes) -> List[str]:
        samplelist = []
        logs = create_splits_logs(split=split, nusc=db)

        samplelist_all = [rec for rec in db.sample if 'LIDAR_TOP' in rec['data'].keys()]
        samplelist = [
            rec['token'] for rec in samplelist_all
            if db.get('log', db.get('scene', rec['scene_token'])['log_token'])['logfile'] in logs
        ]

        return samplelist

    @staticmethod
    def render_global2local(db: NuScenes, global2local: dict) -> dict:
        labelmap = {}

        for category in [category['name'] for category in db.category]:
            match = [template for template in global2local if category.startswith(template)]
            assert len(match) < 2, "Each category should match at most 1 template"
            if len(match) == 1:
                labelmap[category] = global2local[match[0]]
...
def create_splits_logs(split: str, nusc: 'NuScenes') -> List[str]:
    ...
    assert split in scene_splits.keys(), 'Requested splits {} which is not a known nuScenes splits.'.format(split)
    ...
    if split in {'train', 'val', 'train_detect', 'train_track'}:
        ...
        raise ValueError('Requested splits {} which this function cannot map to logs.'.format(split))
...
def create_splits_scenes(verbose: bool = False) -> Dict[str, List[str]]:
    ...
    assert len(all_scenes) == 1000 and len(set(all_scenes)) == 1000
